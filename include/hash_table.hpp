...
struct HashTableIterator {
    ...
    Self& operator++() {
        ...
        if (node_->next_) {
            ...
            if (index == bucket_num) node_ = nullptr; // 结束
        }
        return *this;
    }
...
public:
    ...
    iterator begin() {
        ...
        for (int i = 0; i < tables_.size(); ++i) {
            if (tables_[i]) { cur = tables_[i]; break; }
        }
    ...
    {
        if (tables_.empty()) return iterator(nullptr,this);
    ...
    std::pair<iterator, bool> insert(const T& data) {
        ...
        if (it_pair != end()) return {it_pair, false};
